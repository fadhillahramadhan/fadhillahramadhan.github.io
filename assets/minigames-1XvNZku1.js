...
`,n({icons:e,nameAttr:`data-lucide`}),document.addEventListener(`DOMContentLoaded`,()=>{let r=100,i=new Set,a=()=>{let e=new Date;document.getElementById(`time-display`).textContent=e.toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`}),document.getElementById(`date-display`).textContent=e.toLocaleDateString()};setInterval(a,1e3),a();let o=document.getElementById(`start-btn`),s=document.getElementById(`start-menu`);o.addEventListener(`click`,e=>{e.stopPropagation(),s.classList.toggle(`open`)}),document.addEventListener(`click`,()=>{s.classList.remove(`open`)}),s.addEventListener(`click`,e=>e.stopPropagation());let c=e=>{let t=document.getElementById(`window-${e}`);if(t){r++,t.style.zIndex=r,document.querySelectorAll(`.taskbar-app`).forEach(e=>e.classList.remove(`active`));let n=document.getElementById(`task-${e}`);n&&n.classList.add(`active`)}},l=t=>{let r=document.getElementById(`window-${t}`);if(r.classList.contains(`minimized`)){if(r.classList.remove(`minimized`),r.style.pointerEvents=`auto`,r.style.opacity=`1`,r.style.transform=`none`,c(t),!i.has(t)){i.add(t);let r=document.getElementById(`taskbar-apps`),a=t===`breach`?`terminal-square`:t===`bird`?`bird`:`layers`,o=t===`breach`?`#ef4444`:t===`bird`?`#eab308`:`#3b82f6`,s=document.createElement(`div`);s.className=`taskbar-app active`,s.id=`task-${t}`,s.innerHTML=`<i data-lucide="${a}" width="20" height="20" color="${o}"></i>`,s.addEventListener(`click`,()=>l(t)),r.appendChild(s),n({icons:e,nameAttr:`data-lucide`})}}else{r.classList.add(`minimized`),r.style.pointerEvents=`none`;let e=document.getElementById(`task-${t}`);e&&e.classList.remove(`active`)}},u=e=>{let t=document.getElementById(`window-${e}`);t.classList.add(`minimized`),t.style.pointerEvents=`none`,i.delete(e);let n=document.getElementById(`task-${e}`);n&&n.remove()};document.querySelectorAll(`.desktop-icon`).forEach(e=>{e.addEventListener(`dblclick`,()=>{l(e.dataset.app)}),e.addEventListener(`click`,()=>{window.innerWidth<=768&&l(e.dataset.app)})}),document.querySelectorAll(`.os-window`).forEach(e=>{e.addEventListener(`mousedown`,()=>c(e.id.replace(`window-`,``)))}),document.querySelectorAll(`.os-control-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.action,r=e.dataset.window,i=document.getElementById(`window-${r}`);n===`close`&&u(r),n===`minimize`&&l(r),n===`maximize`&&i.classList.toggle(`maximized`)})}),document.querySelectorAll(`.os-titlebar`).forEach(e=>{let t=!1,n,r,i,a,o=e.parentElement;e.addEventListener(`mousedown`,e=>{o.classList.contains(`maximized`)||(t=!0,n=e.clientX,r=e.clientY,i=o.offsetLeft,a=o.offsetTop,c(o.id.replace(`window-`,``)))}),document.addEventListener(`mousemove`,e=>{if(!t)return;let s=e.clientX-n,c=e.clientY-r;o.style.left=`${i+s}px`,o.style.top=`${a+c}px`}),document.addEventListener(`mouseup`,()=>{t=!1})});let d=document.getElementById(`hack-btn`),f=document.getElementById(`hack-progress`),p=document.getElementById(`hack-status`),m=0,h;d&&d.addEventListener(`click`,()=>{if(!(m>=100)){if(m+=8,m>100&&(m=100),f.style.width=m+`%`,t({targets:d,scale:[.95,1],duration:200,easing:`easeOutQuad`}),m===100){p.textContent=`ACCESS GRANTED. Welcome to the mainframe.`,p.style.color=`#10b981`,d.style.background=`var(--orange)`,d.style.color=`#fff`,d.textContent=`SYSTEM_HACKED`,d.style.cursor=`default`,h&&clearInterval(h);return}h||=(p.textContent=`Warning: Tracing connection...`,p.style.color=`#ef4444`,setInterval(()=>{m>0&&(m-=3,m<0&&(m=0),f.style.width=m+`%`),m===0&&(p.textContent=`Connection lost. Try again.`,p.style.color=`var(--text-muted)`,clearInterval(h),h=null)},200))}});let g=document.getElementById(`cyber-bird-canvas`),_=g?g.getContext(`2d`):null,v=document.getElementById(`cyber-bird-start`),y=document.getElementById(`cyber-bird-overlay`),b=document.getElementById(`cyber-bird-score`);if(g&&_&&v){let e={x:50,y:150,velocity:0,gravity:.3,jump:-5,size:10},n=[],r=0,i=0,a=!1,o,s=()=>{_.fillStyle=`#eab308`,_.beginPath(),_.arc(e.x,e.y,e.size,0,Math.PI*2),_.fill()},c=()=>{_.fillStyle=`#10b981`,n.forEach(e=>{_.fillRect(e.x,0,e.width,e.top),_.fillRect(e.x,g.height-e.bottom,e.width,e.bottom)})},l=0,u=d=>{if(!a)return;o=requestAnimationFrame(u),l||=d;let p=d-l;if(!(p<1e3/60)){if(l=d-p%(1e3/60),e.velocity+=e.gravity,e.y+=e.velocity,(e.y+e.size>g.height||e.y-e.size<0)&&f(),r%90==0){let e=Math.random()*(g.height-120-40)+20,t=g.height-120-e;n.push({x:g.width,width:30,top:e,bottom:t,passed:!1})}n.forEach(n=>{n.x-=2,e.x+e.size>n.x&&e.x-e.size<n.x+n.width&&(e.y-e.size<n.top||e.y+e.size>g.height-n.bottom)&&f(),n.x+n.width<e.x&&!n.passed&&(i++,b.textContent=i,n.passed=!0,t({targets:b,scale:[1.5,1],duration:300,easing:`easeOutQuad`}))}),n=n.filter(e=>e.x+e.width>0),_.clearRect(0,0,g.width,g.height),c(),s(),r++}},d=t=>{a&&(t&&t.preventDefault(),e.velocity=e.jump)},f=()=>{a=!1,cancelAnimationFrame(o),y.style.display=`flex`,y.querySelector(`h3`).textContent=`SYSTEM CRASH`,y.querySelector(`h3`).style.color=`#ef4444`,y.querySelector(`p`).textContent=`Final Score: `+i,v.textContent=`REBOOT`};v.addEventListener(`click`,()=>{e={x:50,y:150,velocity:0,gravity:.3,jump:-5,size:10},n=[],i=0,r=0,b.textContent=`0`,y.style.display=`none`,a=!0,l=performance.now(),u(performance.now()),d()}),g.addEventListener(`mousedown`,d),g.addEventListener(`touchstart`,d),window.addEventListener(`keydown`,e=>{e.code===`Space`&&a&&d(e)}),_.clearRect(0,0,g.width,g.height),s()}let x=document.getElementById(`memory-board`),S=document.getElementById(`memory-score`),C=document.getElementById(`memory-moves`),w=document.getElementById(`memory-restart`);if(x){let r=[`cpu`,`database`,`cloud`,`lock`,`server`,`shield`,`wifi`,`code`],i=[...r,...r],a=0,o=0,s=[],c=!1,l=e=>{for(let t=e.length-1;t>0;t--){let n=Math.floor(Math.random()*(t+1));[e[t],e[n]]=[e[n],e[t]]}return e},u=()=>{x.innerHTML=``,l(i).forEach(e=>{let t=document.createElement(`div`);t.classList.add(`memory-card`),t.dataset.icon=e,t.innerHTML=`
          <div class="memory-card-inner">
            <div class="memory-card-front"></div>
            <div class="memory-card-back"><i data-lucide="${e}" width="24" height="24"></i></div>
          </div>
        `,t.addEventListener(`click`,d),x.appendChild(t)}),n({icons:e,nameAttr:`data-lucide`})};function d(){c||this===s[0]||this.classList.contains(`matched`)||(this.classList.add(`flipped`),s.push(this),s.length===2&&(a++,C.textContent=a,f()))}function f(){let[e,n]=s;e.dataset.icon===n.dataset.icon?(e.classList.add(`matched`),n.classList.add(`matched`),o++,S.textContent=o,s=[],o===8&&setTimeout(()=>{w.style.display=`block`,t({targets:`.memory-card`,rotateY:`+=360`,duration:1e3,delay:t.stagger(100)})},500)):(c=!0,setTimeout(()=>{e.classList.remove(`flipped`),n.classList.remove(`flipped`),s=[],c=!1},1e3))}w.addEventListener(`click`,()=>{a=0,o=0,S.textContent=o,C.textContent=a,w.style.display=`none`,u()}),u()}});